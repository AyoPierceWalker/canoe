...
void CO2WallInteraction(MeshBlock *pmb, Real const time, Real const dt,
                        AthenaArray<Real> const &w, AthenaArray<Real> const &r,
                        AthenaArray<Real> const &bcc, AthenaArray<Real> &u,
                        AthenaArray<Real> &s) {
  int js = pmb->js;
  ...
  Real p_CO2, drhoH2O, drhoH2, drhoCO2;
  Real Tw, Pw, Ta, z, csw_co2, csa_co2, KE;
  ...
  for (int jj = 0; jj <= 1; ++jj) {
    ...
    if ((x2f_left - x2min < pmb->pcoord->dx2f(js)) || (x2max - x2f_right < pmb->pcoord->dx2f(je))) {
        for (int k = pmb->ks; k <= pmb->ke; ++k)
          ...
          p_CO2 = (
            pmb->phydro->w(IDN, k, jw, i) * pmb->phydro->w(iCO2, k, jw, i)
            * Rd * Ta / pthermo->GetMuRatio(iCO2)
          );
          ...
          csa_co2 = sqrt(2 * M_PI * Rd * Ta / pthermo->GetMuRatio(iCO2));

          drhoCO2 = dt * (- p_CO2/csa_co2) / pmb->pcoord->dx2f(jw);

          u(iCO2, k, jw, i) += drhoCO2;

          if (drhoCO2<0) {
              KE= 0.5f*(pmb->phydro->w(IVX,k,jw,i)*pmb->phydro->w(IVX,k,jw,i)
                      + pmb->phydro->w(IVY,k,jw,i)*pmb->phydro->w(IVY,k,jw,i)
                      + pmb->phydro->w(IVZ,k,jw,i)*pmb->phydro->w(IVZ,k,jw,i));
              u(IEN, k, jw, i) += drhoCO2 * (
                KE + (Rd / (gammad - 1.)) * pthermo->GetCvRatioMass(iCO2) * Ta
              );
              u(IVZ, k, jw, i) += drhoCO2 * pmb->phydro->w(IVZ, k, jw, i);
              u(IVY, k, jw, i) += drhoCO2 * pmb->phydro->w(IVY, k, jw, i);
              u(IVX, k, jw, i) += drhoCO2 * pmb->phydro->w(IVX, k, jw, i);
          } else {
              u(IEN, k, jw, i) += drhoCO2 * (
                (Rd / (gammad - 1.)) * pthermo->GetCvRatioMass(iCO2) * Tw
              );
          }
       }
    }
...
void Forcing(MeshBlock *pmb, Real const time, Real const dt,
  ...
  // CO2WallInteraction(pmb, time, dt, w, r, bcc, u, s);
}
